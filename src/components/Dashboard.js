...
function Dashboard() {
  ...
  useEffect(() => {
    socket.on("new_threat", (newThreat) => {
      setThreats((prevThreats) => [newThreat, ...prevThreats].slice(0, 5)); // Keep recent 5 threats
      setChartData((prevData) => [
        ...prevData.slice(-9), // Keep last 9 data points
        { name: newThreat.timestamp.split(" ")[1], threats: prevData.length + 1 },
      ]);
    });
